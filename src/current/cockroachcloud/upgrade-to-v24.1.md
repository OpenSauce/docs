...
- [Splits]({% link {{ page.version.version }}/architecture/distribution-layer.md %}#range-splits) no longer hold [latches]({% link {{ page.version.version }}/architecture/distribution-layer.md %}#latch-manager) for time proportional to the range size while computing [MVCC]({% link {{ page.version.version }}/architecture/storage-layer.md %}#mvcc) statistics. Instead, MVCC statistics are pre-computed before the critical section of the split. As a side effect, the resulting statistics are no longer 100% accurate because they may correctly distribute writes concurrent with the split. To mitigate against this potential inaccuracy, and to prevent the statistics from drifting after successive splits, the existing stored statistics are re-computed and corrected if needed during the non-critical section of the split. [#119894](https://github.com/cockroachdb/cockroach/pull/119894)
